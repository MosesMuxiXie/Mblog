const ALLOWED_TAGS = new Set([
  ...
  's', 'small', 'span', 'strong', 'table', 'tbody', 'td', 'th', 'thead',
  'tr', 'u', 'ul'
]);
const VOID_TAGS = new Set(['br', 'hr', 'img']);
const ALLOWED_CLASSES = new Set([
  'insert-attachment', 'insert-attachment-copy', 'insert-attachment-icon',
  'insert-button', 'insert-card', 'insert-card-info', 'insert-card-tip',
  'insert-card-warning', 'insert-column', 'insert-columns', 'insert-date',
  'insert-signature', 'insert-table'
]);
const ATTACHMENT_DATA_PATTERN = /^data:(?:application\/pdf|text\/(?:plain|csv)|application\/(?:msword|vnd\.ms-excel|vnd\.ms-powerpoint|vnd\.openxmlformats-officedocument\.(?:wordprocessingml\.document|spreadsheetml\.sheet|presentationml\.presentation)|zip));base64,[a-z0-9+/=\s]+$/i;
...
function safeUrl(value, allowImageData = false, allowAttachmentData = false) {
  ...
  if (allowAttachmentData && url.length <= 720000 && ATTACHMENT_DATA_PATTERN.test(url)) {
    return url.replace(/\s/g, '');
  }
  try {
...
function safeDownloadName(value) {
  ...
  return /^[\p{L}\p{N}\s._()+-]{1,160}$/u.test(name) ? name : '';
}
...
function sanitizeTag(token) {
  ...
  const classes = attribute('class')
    .split(/\s+/)
    .filter(className => ALLOWED_CLASSES.has(className));
  if (classes.length) outputAttributes.push(`class="${classes.join(' ')}"`);

  if (tag === 'a') {
    const downloadName = safeDownloadName(attribute('download'));
    const href = safeUrl(attribute('href'), false, Boolean(downloadName));
    if (href) outputAttributes.push(`href="${escapeAttribute(href)}"`);
    if (downloadName && href.startsWith('data:')) {
      outputAttributes.push(`download="${escapeAttribute(downloadName)}"`);
    } else if (attribute('target') === '_blank') {
...
function htmlToText(value) {
  return decodeBasicEntities(String(value || '')
    .replace(/<(?:br|\/p|\/div|\/h[1-3]|\/li|\/blockquote|\/td|\/th|\/tr)\b[^>]*>/gi, '\n')
    .replace(/<[^>]+>/g, '')
    .replace(/\n{3,}/g, '\n\n'))
    .trim()
    .slice(0, 100000);
